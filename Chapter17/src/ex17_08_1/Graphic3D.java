...
/**
 * 去http://java.sun.com/products/java-media/3D/download.html下载Java 3D的API，然后安装本程序将由浅入深地介绍多个Java 3D的例子
 */
...
public class Graphic3D {
	 ...
	 * 这个例子在每个坐标轴上显示不同物体
	 */
	static class TransformPosition {
		public TransformPosition() {
			 ...
			 * 移动物体被称为“变换(transformation)”，所以你要使用的类是TransformGroup和Transform3D
	...
	static class PictureBall {
		public PictureBall() {
			...
			// 建立纹理贴图
			// 纹理装载器(TextureLoader)类允许装载一个图片作为纹理，图片的尺寸必须是2的幂，如：128像素宽256像素高
			// 当载入图片后也可以指定想要如何使用图片。如：RGB使用图片的颜色，LUMINANCE使图片显示为黑白
			TextureLoader loader = new TextureLoader("src/ex17_08_1/google_translate.jpg", new Container());
			...
			// 建立纹理属性
			// 可以用REPLACE，BLEND或DECAL代替MODULATE
