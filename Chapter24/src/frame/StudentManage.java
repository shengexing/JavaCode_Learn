...
class StudentManage extends JFrame implements ActionListener {
	...
	JMenuBar student_bar = new JMenuBar();			// 创建一个菜单栏对象
	...
	JMenuItem basicQuery_item = new JMenuItem("基本信息查询");		// 创建一个文字标签为“基本信息”的菜单项对象
	JMenuItem gradeQuery_item = new JMenuItem("成绩查询");			// 创建一个文字标签为“成绩查询”的菜单项对象
	
	JMenuItem exitOther_item = new JMenuItem("退出");				// 创建一个文字标签为“退出”的菜单项对象
	
	/** 无参的构造器*/
	public StudentManage() {
		this.setTitle("学生基本信息");					// 设置窗口标题
		this.setLayout(new CardLayout());		// 设置窗口布局管理器
		this.setJMenuBar(student_bar);				// 将菜单栏组件添加到容器
		
		student_bar.add(info_menu);			// 	将信息菜单添加到菜单栏
		student_bar.add(query_menu);			// 将查询菜单添加到菜单栏
		student_bar.add(other_meun);			// 将其他菜单添加到菜单栏
		...
		query_menu.add(gradeQuery_item);		// 将成绩查询菜单项添加到查询菜单
		
		query_menu.addActionListener(this);				// 给查询菜单添加监听器
		basicQuery_item.addActionListener(this);			// 给基本信息查询菜单项添加监听器
		gradeQuery_item.addActionListener(this);			// 给成绩查询菜单项添加监听器
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(source == addInfo_item)
			new AddStudent();			// 调用主窗体
		
		// 处理“删除信息”事件
		if(source == deleteInfo_item)
			new DeleteStudent();		// 调用主窗体
		
		// 处理“修改信息”事件
		if(source == updateInfo_item)
			new UpdateStudent();		// 调用主窗体
		
		// 处理“基本信息查询”事件
		if(source == basicQuery_item)
			new StudentSet();				// 调用主窗体
