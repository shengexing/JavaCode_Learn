...
public class UpdateTeacher extends JFrame implements ActionListener {
	...
	JLabel head_label = new JLabel("教师基本信息", JLabel.CENTER);		// 使用文本创建一个“头部”标签对象
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(source == update_button) {
			String number = number_field.getText(), 		// 将“学号”文本框中包含的文本传给字符串number
					...
					post = post_field.getText(), 				// 将“班级”文本框中包含的文本传给字符串sclass
			...
			try {
				...
				if(rs.next()) {		// 判断结果是否存在
					sql = "UPDATE teacher set name='"+ name + "', sclass='"
							+ post + "', sex=''" + sex + "', academy=''" + academy + "', birthday=''" + birthday + "'";		// 修改一组数据
					...
					JOptionPane.showMessageDialog(null, "该学生不存在！");		// 通过showMessageDialog()方法打印信息
				}
			} catch(Exception e1) {
